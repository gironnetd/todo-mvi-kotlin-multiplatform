        ...
        ObservableTransformer<StatisticsAction.LoadStatisticsAction, StatisticsResult.LoadStatisticsResult> = { actions in
            return actions.flatMap { action in
                ...
                return RxSwift.Single.from(self.tasksRepository.getTasks())
                    .asObservable()
                    .map { tasks  in
                        StatisticsResult.LoadStatisticsResult.Success(.init(activeCount: (tasks as! [Task]).filter { task in task.active}.count, completedCount: (tasks as! [Task]).filter { task in !task.active}.count)
                        )
                    }
                    .catch { exception in
    ...
    internal lazy var actionProcessor: ObservableTransformer<StatisticsAction, StatisticsResult> = { actions in
        return Observable.merge(
            ...
            actions.filter { action in action is StatisticsAction.LoadStatisticsAction }
                ...
                .flatMap { value in return RxSwift.Observable.just(StatisticsResult(value)) }
        )
    }
}
