...
class AddEditTaskViewModel {
    ...
    private let intentsSubject: RxSwift.PublishSubject<Intent> = RxSwift.PublishSubject<Intent>()
    private lazy var statesObservable: RxSwift.Observable<ViewState> =  { return self.compose() }()
    lazy var compositeDisposable: RxSwift.CompositeDisposable = CompositeDisposable()
     ...
     */
    private func compose() -> RxSwift.Observable<AddEditTaskViewState> {
                    ...
                    case let result as AddEditTaskResult.PopulateTaskResult.RawValue.Success :
                        if(result.task.active) {
                            return previousState.copy(
                                ...
                                description: result.task.description_
                            )
                        } else {
...
extension AddEditTaskViewModel : MviViewModel {
    ...
    func processIntents(intents: RxSwift.Observable<Intent>) {
        _ = compositeDisposable.insert(intents.subscribe(intentsSubject))
    }
    
    func states() -> RxSwift.Observable<AddEditTaskViewState> {
