    ...
    override func awakeFromNib() {
        ...
        _ = completedBox.rx.tapGesture().when(.recognized).bind { [self] state in
            if(state.state == UIGestureRecognizer.State.ended) {
                ...
                //task?.setValue(!completedBox.isSelected, forKey: "completed")
                
                task = Task(id: task!.id, title: task?.title, description: task?.description_, completed: !completedBox.isSelected)
