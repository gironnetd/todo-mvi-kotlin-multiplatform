...
extension TasksViewController: MviView {
    ...
    func intents() -> RxSwift.Observable<Intent> {
    ...
    private func initialIntent() -> RxSwift.Observable<Intent> {
        return Observable.just(.InitialIntent)
    }
    
    private func refreshIntent() -> RxSwift.Observable<Intent> {
        return refreshIntentPublisher.flatMap { intent in
            return Observable.just(.RefreshIntent(intent))
        }
    }
    
    private func clearCompletedTaskIntent() -> RxSwift.Observable<Intent> {
        return clearCompletedTaskIntentPublisher.flatMap {_ in
            return Observable.just(.ClearCompletedTasksIntent)
        }
    }
    
    private func changeFilterIntent() -> RxSwift.Observable<Intent> {
        return changeFilterIntentPublisher.flatMap { intent in
            return Observable.just(.ChangeFilterIntent(intent))
        }
    }
    
    private func adapterIntents() -> RxSwift.Observable<Intent> {
        return tasksCollectionView.taskToggleObservable.map { task -> TasksViewController.Intent in
            if(task.completed) {
...
class TasksViewController: BaseDrawerViewController   {
    ...
    private let refreshIntentPublisher: RxSwift.PublishSubject = RxSwift.PublishSubject<TasksIntent.RefreshIntent>()
    private let clearCompletedTaskIntentPublisher = RxSwift.PublishSubject<TasksIntent.ClearCompletedTasksIntent>()
    private let changeFilterIntentPublisher = RxSwift.PublishSubject<TasksIntent.ChangeFilterIntent>()
    // Used to manage the data flow lifecycle and avoid memory leak.
    lazy var disposables: RxSwift.CompositeDisposable = CompositeDisposable()
