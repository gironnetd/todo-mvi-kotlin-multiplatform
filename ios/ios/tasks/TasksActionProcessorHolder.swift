...
class TasksActionProcessorHolder {
    ...
    var tasksRepository = Injection.instance.provideTasksRepository()
    ...
    private lazy var loadTasksProcessor:
        ObservableTransformer<TasksAction.LoadTasksAction, TasksResult.LoadTasksResult> = { actions in
            return actions.flatMap { action in
                return RxSwift.Single.from(self.tasksRepository.getTasks(forceUpdate: action.forceUpdate))
                    .asObservable()
                    .map { tasks in
                        TasksResult.LoadTasksResult.Success(.init(tasks: tasks as! [Task], filterType: action.filterType))
                    }
                    .catch { exception in
    ...
    private lazy var activateTaskProcessor:
        ObservableTransformer<TasksAction.ActivateTaskAction, TasksResult.ActivateTaskResult> = { actions in
            return actions.flatMap { action in
                return RxSwift.Completable.from(self.tasksRepository.activateTask(task: action.task))
                    .andThen(RxSwift.Single.from(self.tasksRepository.getTasks()))
                    .asObservable()
                    .flatMap { tasks in
                        return pairWithDelay(
                            TasksResult.ActivateTaskResult.Success(.init(tasks: tasks as! [Task])),
    ...
    private lazy var completeTaskProcessor:
        ObservableTransformer<TasksAction.CompleteTaskAction, TasksResult.CompleteTaskResult> = { actions in
            return actions.flatMap { action in
                return RxSwift.Completable.from(self.tasksRepository.completeTask(task: action.task))
                    .andThen(RxSwift.Single.from(self.tasksRepository.getTasks()))
                    .asObservable()
                    .flatMap { tasks in
                        return pairWithDelay(
                            TasksResult.CompleteTaskResult.Success(.init(tasks: tasks as! [Task])),
    ...
    private lazy var clearCompletedTasksProcessor:
        ObservableTransformer<TasksAction.ClearCompletedTasksAction, TasksResult.ClearCompletedTasksResult> = { actions in
            return actions.flatMap {_ in
                return RxSwift.Completable.from(self.tasksRepository.clearCompletedTasks())
                    .andThen(RxSwift.Single.from(self.tasksRepository.getTasks()))
                    .asObservable()
                    .flatMap { tasks in
                        return pairWithDelay(
                            TasksResult.ClearCompletedTasksResult.Success(.init(tasks: tasks as! [Task])),
    ...
    internal lazy var actionProcessor: ObservableTransformer<TasksAction, TasksResult> = { actions in
        return Observable.merge(
            ...
            actions.filter { action in action is TasksAction.LoadTasksAction }
                ...
                .flatMap { value in return RxSwift.Observable.just(TasksResult(value)) },
